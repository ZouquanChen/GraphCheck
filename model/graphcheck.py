...
class GraphCheck(torch.nn.Module):  # 定义 GraphCheck 主模型，负责把图信息接入冻结 LLM。
    def __init__(self, args, **kwargs):  # 初始化模型、LLM、GNN 和投影头。
        ...
        for i in range(num_devices):  # 逐张卡读取显存容量并设置可用上限。
            ...
            max_memory[i] = (
                f"{max(total_memory - 2, 2)}GiB"  # 预留约 2GiB 给系统和其他张量，至少保留 2GiB 配额。
            )
        ...
        )  # 取出底层 LLM 的词嵌入层，后面要直接把 token 变成 embedding。

        # 方法的主要内容
        # 1. graph_encoder 负责把 claim/doc 两张图编码成节点表示。
        self.graph_encoder = load_gnn_model[
            args.gnn_model_name
        ](  # 根据配置选择具体的 GNN 实现并实例化。
        ...
        self.projector = nn.Sequential(  # 用一个两层 MLP 作为投影头。
            nn.Linear(
                args.gnn_hidden_dim, 2048
            ),  # 先把图向量从 GNN 维度映射到中间维度 2048。
            nn.Sigmoid(),  # 加入非线性，使投影变换不只是单层线性映射。
            nn.Linear(
                2048, self.word_embedding.weight.shape[1]
            ),  # 再映射到 LLM 词向量维度。
        ).to(self.model.device)  # 把投影头也放到 LLM 所在设备。

        self.embed_dim = self.word_embedding.weight.shape[
            1
        ]  # 缓存 LLM embedding 维度，后面创建零向量占位时要用。
    ...
    ):  # 返回一个可直接 with 使用的上下文管理器。
        enable_autocast = self.device != torch.device(
            "cpu"
        )  # 只有在 GPU 环境下才启用 autocast。
    ...
    def encode_graphs(self, data):  # 编码 batch 中的 claim 图和 doc 图。
        ...
        # 1. 得到节点级别表示
        claim_n_embeds, _ = (
            self.graph_encoder(  # 用 GNN 编码 claim 图，得到每个节点的新表示。
                claim_kg.x, claim_kg.edge_index.long(), claim_kg.edge_attr
            )
        )
        doc_n_embeds, _ = (
            self.graph_encoder(  # 用同一个 GNN 编码 doc 图，得到每个节点的新表示。
        ...
        if claim_kg.batch is not None:  # 如果 claim 图来自一个 batched PyG 对象。
            claim_embeds = scatter(
                claim_n_embeds, claim_kg.batch, dim=0, reduce="mean"
            )  # 按图编号做平均池化，得到每张 claim 图的图级向量。
        else:  # 如果当前只有一张图，没有显式的 batch 向量。
            claim_embeds = claim_n_embeds.mean(
                dim=0, keepdim=True
            )  # 直接对节点维做平均，得到单张图的图级向量。

        if doc_kg.batch is not None:  # 如果 doc 图来自 batched PyG 对象。
            doc_embeds = scatter(
                doc_n_embeds, doc_kg.batch, dim=0, reduce="mean"
            )  # 按图编号做平均池化，得到每张 doc 图的图级向量。
        else:  # 如果当前只有一张 doc 图。
            doc_embeds = doc_n_embeds.mean(
                dim=0, keepdim=True
            )  # 直接对节点做平均，得到单张图的图级向量。
        ...
        return (
            claim_embeds,
            doc_embeds,
        )  # 返回 batch 内每条样本对应的 claim/doc 图向量。
    ...
    def forward(self, data):  # 训练阶段的前向传播，最终返回一个标量 loss。
        ...
        )  # 把文本 prompt 编成 token id 序列。
        # 用 tokenizer 编码 batch 中每条样本的目标标签文本，标签通常是 support / unsupport。
        labels = self.tokenizer(
            data["label"], add_special_tokens=False
        )  # 把监督标签文本也编码成 token id 序列。

        # 编码结束标记 EOS，后面会接在标签末尾，表示答案结束。
        eos_tokens = self.tokenizer(
            EOS, add_special_tokens=False
        )  # 单独编码答案结束标记。
        # 编码用户输入结束标记，后面用它把 prompt 和答案区分开。
        eos_user_tokens = self.tokenizer(
            EOS_USER, add_special_tokens=False
        )  # 单独编码用户输入结束标记。
        # 取出 BOS 对应的词向量，后面会把它放在整个 embedding 序列最前面。
        bos_embeds = (
            self.word_embedding(  # 先把 BOS 文本转成 token id，再取对应的词向量。
        ...
        claim_embeds = self.projector(
            claim_embeds
        )  # 把 claim 图向量变换到 LLM embedding 空间。
        ...
        for i in range(batch_size):  # 逐个样本构造训练时真正送入 LLM 的序列。
            ...
            inputs_embeds = (
                self.word_embedding(  # 直接把 token id 映射成 LLM 的输入 embedding。
            ...
            if (
                claim_embeds.size(0) == batch_size
            ):  # 正常情况下，图向量数应与 batch 大小一致。
                claim_embedding = claim_embeds[i].unsqueeze(
                    0
                )  # 取出当前样本的 claim 图向量并补序列维。
            else:  # 如果图向量数量异常。
            ...
            if (
                doc_embeds.size(0) == batch_size
            ):  # 正常情况下，doc 图向量数也应与 batch 对齐。
                doc_embedding = doc_embeds[i].unsqueeze(
                    0
                )  # 取出当前样本的 doc 图向量并补序列维。
            else:  # 如果 doc 图向量数量异常。
            ...
            batch_inputs_embeds.append(
                inputs_embeds
            )  # 保存当前样本的最终 embedding 序列。
            ...
            batch_label_input_ids.append(
                label_input_ids
            )  # 把当前样本的 label 序列加入 batch。

        # 找出当前 batch 中最长的 embedding 序列长度，后面所有样本都对齐到这个长度。
        max_length = max(
            [x.shape[0] for x in batch_inputs_embeds]
        )  # 统计 batch 内最长序列长度。
        ...
        for i in range(batch_size):  # 逐个样本补齐到统一长度。
            ...
            pad_length = (
                max_length - batch_inputs_embeds[i].shape[0]
            )  # 需要在左侧补多少个 pad。
            ...
            batch_attention_mask[i] = [0] * pad_length + batch_attention_mask[
                i
            ]  # 为左侧 pad 位置补 0。
    ...
    def inference(self, data):  # 推理阶段的前向逻辑，返回预测文本而不是 loss。
        ...
        texts = self.tokenizer(
            data["text"], add_special_tokens=False
        )  # 把待预测的输入文本编码成 token 序列。

        # 仍然需要手工准备 BOS 和 padding embedding，因为图向量依旧通过 embedding 注入。
        eos_user_tokens = self.tokenizer(
            EOS_USER, add_special_tokens=False
        )  # 编码用户输入结束标记，用于界定 prompt 结束。
        bos_embeds = (
            self.word_embedding(  # 取出 BOS 对应的 embedding，放在整条序列最前面。
                self.tokenizer(BOS, add_special_tokens=False, return_tensors="pt")
                .input_ids[0]
                .cuda()
            )
        ...
        pad_embeds = (
            self.word_embedding(  # 取出 pad token 的 embedding，供 batch 对齐复用。
                torch.tensor(self.tokenizer.pad_token_id).cuda()
            ).unsqueeze(0)
        )  # 补一个序列维，便于 repeat。
        ...
        claim_embeds = self.projector(
            claim_embeds
        )  # 把 claim 图向量投影到 LLM embedding 空间。
        doc_embeds = self.projector(doc_embeds)  # 把 doc 图向量也投影到同一个空间。
        ...
        for i in range(batch_size):  # 逐条构造推理时的输入序列。
            ...
            inputs_embeds = (
                torch.cat(  # 推理输入同样按 BOS -> claim 图 -> doc 图 -> 文本顺序拼接。
                    [bos_embeds, claim_embedding, doc_embedding, inputs_embeds], dim=0
                )
            )
        ...
        for i in range(batch_size):  # 逐条样本补齐到统一长度。
            ...
            batch_attention_mask[i] = [0] * pad_length + batch_attention_mask[
                i
            ]  # 为左侧 pad 位置补 attention mask=0。

        inputs_embeds = torch.stack(batch_inputs_embeds, dim=0).to(
            self.model.device
        )  # 把推理样本列表堆成 batch embedding 张量。
        attention_mask = torch.tensor(batch_attention_mask).to(
            self.model.device
        )  # 把 attention mask 列表转成张量。
